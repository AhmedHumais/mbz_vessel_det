...
public:
    ...
    {
        int l, r, t, b;
        float r_w = YP.INPUT_W / (img.cols * 1.0);
        float r_h = YP.INPUT_H / (img.rows * 1.0);
        if (r_h > r_w)
        {
            l = bbox[0] - bbox[2] / 2.f;
            r = bbox[0] + bbox[2] / 2.f;
            t = bbox[1] - bbox[3] / 2.f - (YP.INPUT_H - r_w * img.rows) / 2;
            b = bbox[1] + bbox[3] / 2.f - (YP.INPUT_H - r_w * img.rows) / 2;
            l = l / r_w;
            r = r / r_w;
            t = t / r_w;
            b = b / r_w;
        ...
        {
            l = bbox[0] - bbox[2] / 2.f - (YP.INPUT_W - r_h * img.cols) / 2;
            r = bbox[0] + bbox[2] / 2.f - (YP.INPUT_W - r_h * img.cols) / 2;
            t = bbox[1] - bbox[3] / 2.f;
            b = bbox[1] + bbox[3] / 2.f;
            l = l / r_h;
            r = r / r_h;
    ...
    void nms(std::vector<Yolo::Detection> &res, float *output, float conf_thresh, float nms_thresh = 0.5)
    {
        ...
        for (int i = 0; i < output[0] && i < YP.MAX_OUTPUT_BBOX_COUNT; i++)
        {
    ...
    {
        ISliceLayer *s1 = network->addSlice(input, Dims3{0, 0, 0}, Dims3{inch, YP.INPUT_H / 2, YP.INPUT_W / 2}, Dims3{1, 2, 2});
        ISliceLayer *s2 = network->addSlice(input, Dims3{0, 1, 0}, Dims3{inch, YP.INPUT_H / 2, YP.INPUT_W / 2}, Dims3{1, 2, 2});
        ISliceLayer *s3 = network->addSlice(input, Dims3{0, 0, 1}, Dims3{inch, YP.INPUT_H / 2, YP.INPUT_W / 2}, Dims3{1, 2, 2});
        ISliceLayer *s4 = network->addSlice(input, Dims3{0, 1, 1}, Dims3{inch, YP.INPUT_H / 2, YP.INPUT_W / 2}, Dims3{1, 2, 2});
    ...
    std::vector<std::vector<float>> getAnchors(std::map<std::string, Weights> &weightMap, std::string lname)
    {
        ...
        int anchor_len = YP.CHECK_COUNT * 2;
    ...
    IPluginV2Layer *addYoLoLayer(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, std::string lname, std::vector<IConvolutionLayer *> dets)
    {
        ...
        int netinfo[4] = {YP.CLASS_NUM, YP.INPUT_W, YP.INPUT_H, YP.MAX_OUTPUT_BBOX_COUNT};
        ...
        for (size_t i = 0; i < anchors.size(); i++)
        {
            ...
            kernel.width = YP.INPUT_W / scale;
            kernel.height = YP.INPUT_H / scale;
